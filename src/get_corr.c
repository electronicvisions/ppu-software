...
void start() {
    ...
    *signal = 0;
    ...
    while (!*signal) ; 
    cadc_load_causal(VR_CAUSAL_0, loc_a);
    cadc_load_acausal_buffered(VR_ACAUSAL_0, loc_a);
    cadc_load_causal(VR_CAUSAL_1, loc_b);
    cadc_load_acausal_buffered(VR_ACAUSAL_1, loc_b);
    ...
    mailbox_write(4 * sizeof(fxv_array_t), (uint8_t*)param_row, sizeof(fxv_array_t));
