...
ATTRIB_UNUSED static void compute_add_stdp() {
  ...
  fxv_shb(VR_WIN, VR_WEIGHT, 0);
  //fxv_store_array(&tarr, VR_WEIGHT);
  ...
  fxv_splatb(VR_CONST, 1);
 
  fxv_mtach(VR_WOUT_1); // copy content of VR_WOUT_1 to accumulator
  
  fxv_mtacbf(VR_WIN);
  //mtach = move to accumulator halfworld factional
  //mtacbf = move to accumulator byte fractional
   //fxv_addacbfs(VR_TMP, VR_WOUT_1);
   //fxv_store_array(&tarr, VR_TMP);
  //fxv_store_array(&tarr, VR_A);
  
  //fxv_mabfs(VR_WOUT_0, VR_A, VR_LAMBDA); // upper 8 bits of result of addition go
  // round up for VR_TMP < 0
  fxv_mulbfs(VR_TMP, VR_A, VR_LAMBDA);
  fxv_addbm(VR_TMP_0, VR_TMP, VR_CONST);
  fxv_cmpb(VR_TMP);
  fxv_sel_lt(VR_TMP, VR_TMP, VR_TMP_0);
  //fxv_shb(VR_TMP, VR_TMP, 1);
  
  //~ fxv_shb(VR_TMP_0, VR_TMP, -1);
  //~ fxv_cmpb(VR_TMP);
  //~ fxv_sel_lt(VR_TMP_1, VR_TMP_0, VR_TMP);
  fxv_addbfs(VR_WOUT_0, VR_WIN, VR_TMP);
  ...
  // check if weight greater 64
  fxv_splatb(VR_CONST, 63);
  fxv_subbfs(VR_TMP_1, VR_WOUT, VR_CONST);
  fxv_cmpb(VR_TMP_1);
  fxv_sel_lt(VR_WOUT, VR_CONST, VR_WOUT); 
   
  fxv_shb(VR_WEIGHT, VR_WOUT, 0);
...
void start() {
    ...
    time_base_t t;
    int i, j;
    uint8_t causal[8], acausal[8], diff_save[8], weights[8], tmp[8], tmp2[8];
    /*uint8_t as[as_size];*/
    ...
    while (1) {
        ...
        *signal = 0;
        ...
        for(loc_a=0; loc_a<(*param_max_row)*2; loc_a+=2) {
            ...
            fxv_store_array(&tarr, VR_LAMBDA);
            fxv_mulbfs(VR_TMP, VR_A, VR_LAMBDA);
            fxv_store_array(&tarr2, VR_TMP); 
            sync();
            if (j < 8) {
                causal[j] = c.bytes[0] & 0xff;
                acausal[j] = a.bytes[0] & 0xff;
                diff_save[j] = diff.bytes[0] & 0xff;
                weights[j] = w.bytes[0] & 0xff;
                tmp[j] = tarr.bytes[0] & 0xff;
                tmp2[j] = tarr2.bytes[0] & 0xff;
                j++;
            }
            compute_add_stdp();
...
//~ 
        ...
        //a.bytes[0] = 0xff;
        //~ a.bytes[1] = 0xff;
        mailbox_write(1 * sizeof(fxv_array_t), (uint8_t*)&causal, sizeof(uint8_t)*8);
        mailbox_write(2 * sizeof(fxv_array_t), (uint8_t*)&acausal, sizeof(uint8_t)*8);
        mailbox_write(3 * sizeof(fxv_array_t), (uint8_t*)&diff_save, sizeof(uint8_t)*8);
        mailbox_write(4 * sizeof(fxv_array_t), (uint8_t*)&weights, sizeof(uint8_t)*8);
        mailbox_write(5 * sizeof(fxv_array_t), (uint8_t*)&tmp, sizeof(uint8_t)*8);
        mailbox_write(6 * sizeof(fxv_array_t), (uint8_t*)&tmp2, sizeof(uint8_t)*8);
    }
}
